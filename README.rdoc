...
== Using the Cache
It's fairly easy to enable caching.

    client = Databasedotcom::Client.new :cache => true, :client_id => "sponge", :client_secret => "bob"

Remember, the cache uses Dalli which requires memcached 1.4+. You can check the version with memcached -h. Please note that memcached that Mac OS X Snow Leopard ships with is 1.2.8 and won't work. Install 1.4.x using Homebrew with
